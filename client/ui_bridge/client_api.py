...
class ClientAPI:
    ...
    # ------------------------------------------------------------------
    def configure(self, url: str, token: str | None) -> None:
        self._url = url
        self._token = token
        self.link.configure(url, token)
        self.rl_manager.set_bridge_url(url)
    ...
    def send_rl_reward(self, shot_id: str, hit: bool) -> None:
        """Forward GUI feedback to the RL manager."""
        try:
            self.rl_manager.record_reward(shot_id, hit)
        except Exception:  # noqa: BLE001
            logger.exception("Failed to record RL reward for shot %s", shot_id)

    def _send_rl_cmd(self, cmd: Dict[str, object]) -> None:
        """Bridge RL commands back through the primary transport."""
        self.link.send_command(cmd)  # type: ignore[arg-type]
    ...
    def _handle_frame(self, frame: Dict[str, object]) -> None:
        ...
        try:
            self.rl_manager.handle_frame(frame)  # type: ignore[arg-type]
        except Exception:
            logger.exception("RLManager.handle_frame failed")
    ...
    def _handle_status(self, status: Dict[str, object]) -> None:
        ...
        try:
            self.rl_manager.handle_status(status)  # type: ignore[arg-type]
        except Exception:
            logger.exception("RLManager.handle_status failed")
