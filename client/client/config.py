...
class ClientConfig:
    ...
    def load(cls, path: Path) -> "ClientConfig":
        ...
        # 1. Get the directory of the config.yml file so relative paths in the
        # config are resolved relative to the config file location.
        ...
        # Make directories and model path absolute relative to the config file.
        artifacts_dir = (config_dir / paths_raw.get("artifacts_dir", "artifacts")).resolve()
        logs_dir = (config_dir / paths_raw.get("logs_dir", "logs")).resolve()
        model_path_raw = paths_raw.get("model_path", "rpm_model.json")
        model_path = (config_dir / model_path_raw).resolve()
