...
class RpmModel:
    ...
    def update_from_shot_context(
        self,
        shot_frame: Dict[str, Any],
        *,
        reward: float,
        alpha: float,
        distance_bins: Iterable[Sequence[float]],
        ...
        features = self.extract_shot_features(shot_frame)
        if not features:
            return None
        self.update_residual(
            distance_in=features["distance_in"],
            v_batt_load=features["v_batt_load"],
            rpm_at_fire=features["rpm_at_fire"],
            alpha=alpha,
        ...
        if self.path:
            try:
                ...
                logger.exception("Failed to persist RPM model after reward update")
        return features

    @staticmethod
    def extract_shot_features(shot_frame: Dict[str, Any]) -> Optional[Dict[str, float]]:
        if not isinstance(shot_frame, dict):
            return None
        ...
        if distance is None or voltage is None or rpm is None:
            return None
        return {
...
def _first_float(payload: Dict[str, Any], paths: Sequence[str]) -> Optional[float]:
    for path in paths:
        value = _lookup_path(payload, path)
        number = _coerce_float_opt(value)
...
def _coerce_float_opt(value: object) -> Optional[float]:
    if value is None:
        return None
    try:
        return float(value)
