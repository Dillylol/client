...
class DevControllerApp(tk.Tk):
    ...
    def __init__(
        ...
        self.api = client or ClientAPI(config_path=config_path)
    ...
    def _build_ui(self) -> None:
        ...
        bg_sidebar = "#252526"    # Sidebar/Panel background
        bg_activity = "#333333"   # Activity bar / header background
        bg_input = "#3c3c3c"      # Input field background
        bg_selection = "#264f78"  # Selection background
        bg_button = "#0e639c"     # Primary button background
        bg_button_hover = "#1177bb"
        
        # Foregrounds
        fg_primary = "#cccccc"    # Main text
        fg_secondary = "#858585"  # Subtitles / labels
        ...
        # Labels
        style.configure("App.TLabel", background=bg_main, foreground=fg_primary, font=("Segoe UI", 10))
        style.configure("Sidebar.TLabel", background=bg_sidebar, foreground=fg_primary, font=("Segoe UI", 10))
        style.configure("Title.TLabel", background=bg_main, foreground=fg_white, font=("Segoe UI", 11, "bold"))
        style.configure("SidebarTitle.TLabel", background=bg_sidebar, foreground="#bbbbbb", font=("Segoe UI", 9, "bold"))
        style.configure("StatusValue.TLabel", background=bg_sidebar, foreground=fg_white, font=("Segoe UI", 10))
        style.configure("StatusHeader.TLabel", background=bg_sidebar, foreground=fg_secondary, font=("Segoe UI", 9, "bold"))
        ...
        style.map("Accent.TButton", 
            background=[("active", bg_button_hover), ("pressed", "#094771")]
        )

        # Checkbuttons
        style.configure("App.TCheckbutton", background=bg_main, foreground=fg_primary, font=("Segoe UI", 10))
        ...
        # Notebook (Tabs)
        style.configure("TNotebook", background=bg_activity, borderwidth=0)
        style.configure("TNotebook.Tab", 
            background=bg_activity, 
            foreground="#969696", 
            padding=(12, 8), 
            font=("Segoe UI", 10)
        )
        ...
        style.map("Treeview", 
            background=[("selected", bg_selection)], 
            foreground=[("selected", fg_white)]
        )
        ...
        # Tabs
        notebook_frame = ttk.Frame(right_pane, style="App.TFrame")
        ...
        self.cv_tab = ttk.Frame(self.nb, padding=16, style="App.TFrame")
        self.nb.add(self.cv_tab, text="CAMERA")
        self._build_cv_tab(self.cv_tab)

        self.requests_tab = ttk.Frame(self.nb, padding=16, style="App.TFrame")
        self.nb.add(self.requests_tab, text="COMMANDS")
        self._build_requests_tab(self.requests_tab)
        self.rl_tab = ttk.Frame(self.nb, padding=16, style="App.TFrame")
    ...
    def _build_status_panel(self, parent: ttk.Panedwindow) -> ttk.Frame:
        ...
        ttk.Label(frame, text="Connection", style="SidebarTitle.TLabel").grid(row=0, column=0, sticky="w")
        conn_row = ttk.Frame(frame, style="Sidebar.TFrame")
        conn_row.grid(row=1, column=0, sticky="ew", pady=(6, 12))

        self.status_dot = tk.Canvas(conn_row, width=14, height=14, highlightthickness=0, bg="#252526")
        ...
        token_row = ttk.Frame(frame, style="Sidebar.TFrame")
        ...
        btn_row = ttk.Frame(frame, style="Sidebar.TFrame")
    ...
    def _build_rl_tab(self, frame: ttk.Frame) -> None:
        ...
        pending_frame = ttk.Frame(frame, style="App.TFrame")
        pending_frame.pack(fill=tk.X, pady=(0, 12))
        ttk.Label(pending_frame, text="Shots Awaiting Feedback", style="Title.TLabel").pack(anchor="w", pady=(0, 6))
        self.pending_shots_frame = ttk.Frame(pending_frame, style="App.TFrame")
        self.pending_shots_frame.pack(fill=tk.X, expand=False)
        self.pending_placeholder_label = ttk.Label(
            self.pending_shots_frame,
            text="No pending shots",
            style="App.TLabel",
        )
        self.pending_placeholder_label.pack(anchor="w")
    ...
    def _handle_rl_event(self, event: Dict[str, Any]) -> None:
        ...
        if kind == "status":
            ...
            if evt_type == "shot_pending":
                self._add_pending_shot(payload)
                summary = self._summarize_shot(payload.get("payload") or {})
                self._append_rl_log(ts_str, f"pending {payload.get('shot_id')}: {summary}")
                return
            if evt_type == "reward_recorded":
                shot_id = str(payload.get("shot_id") or "")
                self._remove_pending_shot(shot_id)
                result = "hit" if payload.get("hit") else "miss"
                self._append_rl_log(ts_str, f"reward {shot_id}: {result}")
                return
    ...
    def _add_pending_shot(self, payload: Dict[str, Any]) -> None:
        shot_id = str(payload.get("shot_id") or "")
        if not shot_id or not hasattr(self, "pending_shots_frame"):
            return
        self._remove_pending_shot(shot_id)
        shot_payload = payload.get("payload") or {}
        card = ttk.Frame(self.pending_shots_frame, padding=8, style="App.TFrame")
        card.pack(fill=tk.X, pady=(0, 8))
        ttk.Label(card, text=f"Shot {shot_id}", style="Title.TLabel").pack(anchor="w")
        ttk.Label(card, text=self._summarize_shot(shot_payload), style="App.TLabel").pack(anchor="w", pady=(2, 6))
        session_id = shot_payload.get("session_id")
        if session_id:
            self.rl_session_var.set(str(session_id))

        btn_row = ttk.Frame(card, style="App.TFrame")
        btn_row.pack(anchor="w")
        ttk.Button(btn_row, text="Yes (Scored)", command=lambda s=shot_id: self._send_shot_feedback(s, True)).pack(side=tk.LEFT)
        ttk.Button(btn_row, text="No (Missed)", command=lambda s=shot_id: self._send_shot_feedback(s, False)).pack(side=tk.LEFT, padx=(8, 0))

        self.pending_shot_widgets[shot_id] = card
        self._update_pending_placeholder()

    def _remove_pending_shot(self, shot_id: str) -> None:
        if not shot_id:
            return
        card = self.pending_shot_widgets.pop(shot_id, None)
        if card is not None:
            card.destroy()
        self._update_pending_placeholder()

    def _update_pending_placeholder(self) -> None:
        placeholder = getattr(self, "pending_placeholder_label", None)
        if placeholder is None:
            return
        if self.pending_shot_widgets:
            placeholder.pack_forget()
        else:
            if not placeholder.winfo_manager():
                placeholder.pack(anchor="w")

    def _send_shot_feedback(self, shot_id: str, hit: bool) -> None:
        self.api.send_rl_reward(shot_id, hit)
        verdict = "hit" if hit else "miss"
        self.status_bar_var.set(f"Recorded {verdict} for shot {shot_id}")

    def _summarize_shot(self, shot: Dict[str, Any]) -> str:
        ctx = shot.get("context") if isinstance(shot.get("context"), dict) else {}
        rpm = shot.get("rpm_at_fire")
        battery = shot.get("battery_v") or ctx.get("battery_v") or ctx.get("sim_battery_v")
        parts: List[str] = []
        try:
            if rpm is not None:
                parts.append(f"{float(rpm):.0f} rpm")
        except (TypeError, ValueError):
            pass
        try:
            if battery is not None:
                parts.append(f"{float(battery):.2f} V")
        except (TypeError, ValueError):
            pass
        if not parts:
            return json.dumps(shot, ensure_ascii=False, default=str)[:60]
        return " · ".join(parts)
